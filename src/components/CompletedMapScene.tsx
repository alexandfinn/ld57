...
export const CompletedMapScene = ({ isVisible, onRestart }: CompletedMapSceneProps) => {
  ...
  const downloadMap = () => {
    ...
    // Clear the canvas
    ctx.clearRect(0, 0, mapCanvasRef.current.width, mapCanvasRef.current.height);
    ...
    mapImage.onload = () => {
      // Draw the base map
      if (!mapCanvasRef.current) return;
      ctx.drawImage(mapImage, 0, 0, mapCanvasRef.current.width, mapCanvasRef.current.height);
      ...
      if (mapData) {
        ...
        drawingImage.onload = () => {
          ...
          if (!mapCanvasRef.current) return;
          ctx.globalAlpha = 0.8;
          ctx.drawImage(drawingImage, 0, 0, mapCanvasRef.current.width, mapCanvasRef.current.height);
